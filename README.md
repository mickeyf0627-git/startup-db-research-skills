...
④生成後に `update_matching_in_excel.py --check-only` で **[品質]警告** を検出する。検出対象は、a/b/c判定の明記なし・思考根拠が薄い・全行同一スコア・要約13列の欠落・概要が定型文・**採用障壁/改善点/主要リスク等に定型プレースホルダー（「詳細分析」「予定」等）が残る**行。警告が出たら最大3回の品質改善パスで是正し、警告0を目指す（事実が不明な欄は仮テキストで埋めず空欄にする）。
